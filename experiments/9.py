...
def _extract_specs(target, mode):
    ...
    result_log = (
        f"[bold magenta]Return code:[/bold magenta] {result.returncode}\n\n"
        f"[bold magenta]Stdout:[/bold magenta]\n{result.stdout}\n\n"
        f"[bold magenta]Stderr:[/bold magenta]\n{result.stderr}"
    )
    rich.get_console().print(Panel(result_log))
...
def _verify_extracted_specs(target_dir, specs):
    ...
    for i, spec in enumerate(specs):
        rich.print(f"Post-verification: {i}/{specs_count}")
        ...
        try:
            ...
            rich.print("[red]AttributeError; ignored!")
            holds_not_specs.add(spec)
            continue
        ...
        if (
            ...
            policy_holds = any(
                t.disposition in ("ACCEPTED", "EXITS_NETWORK") for t in result.Traces[0]
            )
        else:
            policy_holds = False

        if policy_holds:
            holds_specs.add(spec.replace("HOLDSNOT", "HOLDS"))
        elif "HOLDSNOT" not in spec:
            holds_specs.add(spec.replace("HOLDS", "HOLDSNOT"))
        else:
            holds_not_specs.add(spec)
...
def run_network(network, algorithm, target, progress, task):
    ...
    target_label = algorithm.capitalize()
    ...
    _phase("Loading forwarding info...")
    nethide_dir = network_dir / NETHIDE_NAME
    ...
    # Evaluate NetHide
    _phase("[NetHide] Extracting original specs...")
    nh_origin_specs = _extract_specs(origin_dir, "nethide-specs")
    _phase("[NetHide] Extracting anonymized specs...")
    ...
    _phase(f"[{target_label}] Extracting original specs...")
    origin_specs = _extract_specs(origin_dir, "full-policies")
    if PROTOCOL_MAPPING[network] == "bgp":
        _phase(f"[{target_label}] Post-verifying original specs...")
        origin_specs, _ = _verify_extracted_specs(origin_dir, origin_specs)
    _phase(f"[{target_label}] Extracting anonymized specs...")
    target_specs = _extract_specs(target_dir, "full-policies")
    if PROTOCOL_MAPPING[network] == "bgp":
        _phase(f"[{target_label}] Post-verifying anonymized specs...")
        target_specs, _ = _verify_extracted_specs(target_dir, target_specs)
    ...
    _phase(
        "[green]Done[/green]"
        f" | {algorithm}: {result[algorithm]['kept_ratio']:.2%}"
